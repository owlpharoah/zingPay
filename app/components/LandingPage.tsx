...
interface LandingPageProps {
  /** Whether the waitlist gate is on (read from WAITLIST_MODE on the server). */
  waitlistMode: boolean;
}

export default function LandingPage({ waitlistMode }: LandingPageProps) {
  ...
  // CTA destination + copy.
  // - Waitlist mode ON: every CTA points at the waitlist and reads "Join waitlist".
  // - Waitlist mode OFF: CTAs open the app. A connected wallet that's already
  //   registered jumps straight to Send; everyone else starts at ID Register.
  //   The header/footer CTAs read "Open App"; the in-page CTAs read "Try it now".
  const appHref = registered && publicKey ? "/send" : "/register";
  const ctaHref = waitlistMode ? "/waitlist" : appHref;
  const appLabel = waitlistMode ? "Join waitlist" : "Open App";
  const tryLabel = waitlistMode ? "Join waitlist" : "Try it now";
    ...
    <div className="w-full overflow-x-hidden font-[outfit]">
      <header className="bg-[#0B2818]">
        <div className="mx-auto flex max-w-7xl max-lg:justify-between gap-4 px-4 py-5 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-10">
          ...
          <Link href={ctaHref} className="hidden lg:block w-full rounded-3xl bg-[#192FFD] px-5 py-2 text-center sm:w-auto">
            <p className="text-lg font-semibold text-white">{appLabel}</p>
          </Link>
          ...
          <div>
            ...
            <FadeIn delay={0.4}>
              <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
                <Link href={ctaHref} className="flex h-12 items-center justify-center rounded-3xl border-2 border-[#0B2818] bg-[#192FFD] px-6 shadow-[0px_4px_0px_0px_#0B2818]">
                  <p className="text-lg font-semibold text-white">{tryLabel}</p>
                </Link>
      ...
      <section className="relative -top-130 sm:-top-200 space-y-4 pb-12 z-1 md:-top-210">
        ...
        <div className="relative left-1/2 right-1/2 top-26 sm:top-60 md:top-50">
          ...
          <Link href={ctaHref} className="relative -top-8 text-sm sm:-top-15 sm:left-3 w-fit rotate-6 items-center justify-center rounded-3xl  bg-[#192FFD] px-5 py-3 sm:text-xl font-semibold text-white shadow-[0px_4px_0px_0px_#0B2818] flex">
            <p>{tryLabel}</p>
          </Link>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-10" id="security">
        <FadeIn>
          ...
          <div className="flex -mb-8 relative -top-18 left-4 sm:left-45 sm:-top-44">
            <Link href={ctaHref} className="font-outfit flex w-fit items-center justify-center rounded-3xl border-white bg-[#192FFD] px-3 py-1 text-xs sm:text-xl sm:px-10 sm:py-4 font-semibold text-white shadow-[0px_4px_0px_0px_#0B2818]">
              <p>{tryLabel}</p>
            </Link>
          </div>
        </FadeIn>
      ...
      <footer className="bg-[#0B2818] px-4 py-10 text-center sm:px-6 lg:px-10">
        ...
        <Link href={ctaHref} className="mt-8 inline-flex rounded-3xl bg-[#192FFD] px-6 py-3 border-white border">
          <p className="text-xl font-semibold text-white">{appLabel}</p>
        </Link>
      </footer>
    </div>
  );
